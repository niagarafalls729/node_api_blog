...
async function testRealtimeUrls() {
  const urls = [
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      $('tr.ub-content').slice(0, 10).each((j, el) => {
        const $el = $(el);
      ...
      const targetPost = posts.find(post => 
        post.title.includes('한화') && post.title.includes('김나연') && post.title.includes('수영복')
      );
      
      if (targetPost) {
        console.log(`🎯🎯🎯 목표 게시글 발견! 🎯🎯🎯`);
        console.log(`    ID: ${targetPost.id}, 제목: ${targetPost.title}`);
        return; // 발견하면 중단
      } else {
